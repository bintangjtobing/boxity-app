                ...
                <div class="breadcrumb-main user-member justify-content-sm-between ">
                    <div class=" d-flex flex-wrap justify-content-center breadcrumb-main__wrapper">
                        <div class="d-flex align-items-center user-member__title justify-content-center mr-sm-25">
                            <h4 class="text-capitalize fw-500 breadcrumb-title">New Shipping Confirmation</h4>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        title() {
            return `New Shipping Confirmation`;
        },
        data() {
            return {
                // Page Info
                titleItemDescription: 'Add some items on Shipping Confirmations.',
                isVisibleSave: false,
        ...
        methods: {
            ...
            async addToList() {
                ...
                await axios.post('/api/item-purchase', this.itemAdd).then(response => {
                    Swal.fire({
                        ...
                        text: 'Success add item on Shipping Confirmation table',
                    });
                    this.itemAdd = {
                        item_code: '',
                        qtyOrdered: '0',
            ...
            async modifyItemPurchase() {
                ...
                await axios.patch('/api/item-purchase/' + this.itemAdd.id, this.itemAdd).then(response => {
                    Swal.fire({
                        icon: 'success',
                        title: 'Congratulations',
                        text: 'Success modify item on Shipping Confirmation table',
                    });
                    this.itemAdd = {
                        item_code: '',
                        qtyOrdered: '0',
                ...
                this.titleItemDescription = 'Add some items on Shipping Confirmations.';
            ...
            async submitHandle() {
                this.$Progress.start();
                await axios.post('/api/shipping-confirmation', this.purchaseOrderData).then(response => {
                    this.loadData();
                    Swal.fire({
                        icon: 'success',
                        title: 'Congratulations',
                        text: 'Success New Shipping Confirmation',
                    });
            ...
            async deleteItemPurchasing(id) {
                const result = await Swal.fire({
                    title: 'Delete item Shipping Confirmation?',
                    showCancelButton: true,
                    cancelButtonColor: '#d33',
                    confirmButtonText: `Delete`,
                });
