    ...
    export default {
        ...
        data() {
            return {
                ...
                dropzoneOptions: {
                    url: '/api/stock-group/images/' + this.$route.params.id,
                    thumbnailWidth: 200,
                    maxFilesize: 2, // MB
                    addRemoveLinks: true,
                    autoDiscover: false,
        ...
        methods: {
            ...
            async loadDataWarehouse() {
                ...
                const imgList = await axios.get('/api/stock-group/album/' + this.$route.params.id);
            ...
            async deleteThisImage(id) {
                ...
                if (result.isConfirmed) {
                    // this.$Progress.start();
                    this.$isLoading(true);
                    await axios.delete('/api/stock-group/images/' + id);
                    this.$isLoading(false);
                    this.loadDataWarehouse();
                    document.getElementById('ding').play();
                    await Swal.fire({
