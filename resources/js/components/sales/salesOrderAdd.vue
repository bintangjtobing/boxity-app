          ...
          >
            ...
            >
              <h4 class="text-capitalize fw-500 breadcrumb-title">
                New Sales Order <span>#{{ purchaseOrderData.po_number }}</span>
              </h4>
            </div>
          </div>
        </div>
            ...
            <div class="form-row">
              <div class="col-lg-12">
                <div class="form-group">
                  ...
                  <textarea
                    class="form-control"
                    v-model="purchaseOrderData.remarks"
                    cols="30"
                    rows="4"
                  ></textarea>
                </div>
...
export default {
  ...
  methods: {
    ...
    generateSONumber() {
      ...
      this.purchaseOrderData.po_number = genSONumber;
    },
    rndStr(len) {
      let blank = "";
      let text = "SO.";
    ...
    async modifyItemList() {
      ...
      await axios
        .patch("/api/po/item-purchase/" + this.itemModify.id, this.itemModify)
        .then((response) => {
          Swal.fire({
            icon: "success",
    ...
    async addToList() {
      this.$Progress.start();
      await axios
        .post("/api/po/item-purchase", this.itemAdd)
        .then((response) => {
          Swal.fire({
            icon: "success",
            title: "Congratulations",
    ...
    async submitHandle() {
      this.$Progress.start();
      await axios
        .post("/api/purchase/order", this.purchaseOrderData)
        .then((response) => {
          this.loadData();
          Swal.fire({
            icon: "success",
